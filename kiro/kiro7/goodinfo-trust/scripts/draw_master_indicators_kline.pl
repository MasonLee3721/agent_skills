...
sub generate_master_indicators_svg {
    my ($code) = @_;
    ...
    my $days_ref = calculate_master_indicators($raw_days_ref);
    my @days = @$days_ref;
    my $count = scalar(@days);
    my $latest = $days[-1];
    ...
    my $w = 1040;
    my $h = 920; # 5 Vertical Subcharts Layout: Main + Vol + KD + MACD/CD + RSI
    ...
    my $kd_top   = 430;  my $kd_h   = 110; # KD(9,3,3)
    my $macd_top = 560;  my $macd_h = 130; # MACD & CD值
    my $rsi_top  = 710;  my $rsi_h  = 110; # RSI(14)
...
<svg width="$w" height="$h" xmlns="http://www.w3.org/2000/svg">
  ...
  <text x="20" y="33" fill="#facc15" font-size="22" font-weight="bold" font-family="Inter, 'Noto Sans TC', sans-serif">$code $name 大滿貫全指標技術分析圖 (含KD/MACD/CD值/RSI)</text>
  <text x="560" y="33" fill="$change_color" font-size="17" font-weight="bold" font-family="Inter, sans-serif">收盤: $latest->{close} $change_str</text>
  <text x="920" y="33" fill="#ffffff" font-size="13" font-weight="bold" font-family="sans-serif">2026/08/14</text>
  ...
  <text x="@{[$w - $pad_right + 5]}" y="@{[$kd_top + $kd_h*0.2 + 4]}" fill="#ef4444" font-size="11" font-weight="bold" font-family="sans-serif">80檔高</text>
  <text x="@{[$w - $pad_right + 5]}" y="@{[$kd_top + $kd_h*0.8 + 4]}" fill="#10b981" font-size="11" font-weight="bold" font-family="sans-serif">20檔低</text>
...
|;
    ...
    # 📌 RIGHT PANEL: MASTER TECHNICAL INDICATOR DASHBOARD
  ...
  <text x="@{[$panel_x + $panel_w/2]}" y="@{[$main_top+25]}" fill="#facc15" font-size="13" font-weight="bold" font-family="sans-serif" text-anchor="middle">👑 大滿貫全指標權威評估</text>
  
  <!-- 1. 150日波段總漲幅 -->
  <text x="@{[$panel_x+10]}" y="@{[$main_top+58]}" fill="#94a3b8" font-size="11" font-weight="bold" font-family="sans-serif">1. 150日總波段漲幅:</text>
  <rect x="@{[$panel_x+10]}" y="@{[$main_top+65]}" width="170" height="24" rx="4" fill="#ff334b"/>
  <text x="@{[$panel_x+95]}" y="@{[$main_top+82]}" fill="#ffffff" font-size="13" font-weight="bold" font-family="sans-serif" text-anchor="middle">@{[sprintf("%+.2f%%", $total_return)]}</text>

  <!-- 2. KD(9,3,3) 轉折評估 -->
  <text x="@{[$panel_x+10]}" y="@{[$main_top+112]}" fill="#94a3b8" font-size="11" font-weight="bold" font-family="sans-serif">2. KD(9,3,3) 轉折訊號:</text>
  <rect x="@{[$panel_x+10]}" y="@{[$main_top+119]}" width="170" height="24" rx="4" fill="$kd_bg"/>
  <text x="@{[$panel_x+95]}" y="@{[$main_top+136]}" fill="#ffffff" font-size="12" font-weight="bold" font-family="sans-serif" text-anchor="middle">$kd_status</text>
  <text x="@{[$panel_x+10]}" y="@{[$main_top+156]}" fill="#facc15" font-size="11" font-weight="bold" font-family="sans-serif">K:@{[sprintf("%.1f",$latest->{k_val})]} / D:@{[sprintf("%.1f",$latest->{d_val})]}</text>

  <!-- 3. MACD / CD值狀態 -->
  <text x="@{[$panel_x+10]}" y="@{[$main_top+182]}" fill="#94a3b8" font-size="11" font-weight="bold" font-family="sans-serif">3. MACD / CD值柱狀體:</text>
  <rect x="@{[$panel_x+10]}" y="@{[$main_top+189]}" width="170" height="24" rx="4" fill="@{[$latest->{osc} >= 0 ? '#ff334b' : '#00e676']}"/>
  <text x="@{[$panel_x+95]}" y="@{[$main_top+206]}" fill="#ffffff" font-size="12" font-weight="bold" font-family="sans-serif" text-anchor="middle">@{[$latest->{osc} >= 0 ? '🔥 CD值紅柱 (多頭強勢)' : '🧊 CD值綠柱 (空頭修正)']}</text>

  <!-- 4. RSI(14) 動能位置 -->
  <text x="@{[$panel_x+10]}" y="@{[$main_top+236]}" fill="#94a3b8" font-size="11" font-weight="bold" font-family="sans-serif">4. RSI(14) 強弱位置:</text>
  <rect x="@{[$panel_x+10]}" y="@{[$panel_x+10]}" width="170" height="22" rx="4" fill="#c084fc"/>
  <text x="@{[$panel_x+95]}" y="@{[$main_top+258]}" fill="#c084fc" font-size="13" font-weight="bold" font-family="sans-serif" text-anchor="middle">RSI: @{[sprintf("%.1f", $latest->{rsi})]} (@{[$latest->{rsi}>70?'強勢超買':$latest->{rsi}>50?'多頭控盤':'弱勢區']})</text>

  <!-- 5. 150日最高極致價 -->
  <text x="@{[$panel_x+10]}" y="@{[$main_top+288]}" fill="#94a3b8" font-size="11" font-weight="bold" font-family="sans-serif">5. 150日最高天價:</text>
  <text x="@{[$panel_x+10]}" y="@{[$main_top+306]}" fill="#facc15" font-size="13" font-weight="bold" font-family="sans-serif">🔥 $max_p 元</text>
  
  <!-- 6. 150日最低支撐價 -->
  <text x="@{[$panel_x+10]}" y="@{[$main_top+332]}" fill="#94a3b8" font-size="11" font-weight="bold" font-family="sans-serif">6. 150日波段底價:</text>
  <text x="@{[$panel_x+10]}" y="@{[$main_top+350]}" fill="#38bdf8" font-size="13" font-weight="bold" font-family="sans-serif">🛡️ $min_p 元</text>

  <!-- 7. 均線大滿貫 -->
  <text x="@{[$panel_x+10]}" y="@{[$main_top+376]}" fill="#94a3b8" font-size="11" font-weight="bold" font-family="sans-serif">7. 均線大滿貫:</text>
  <text x="@{[$panel_x+10]}" y="@{[$main_top+394]}" fill="#10b981" font-size="12" font-weight="bold" font-family="sans-serif">✓ 5MA>20MA>60MA</text>

  <!-- 綜合評分 -->
  <rect x="@{[$panel_x+10]}" y="@{[$main_top+740]}" width="170" height="42" rx="6" fill="#10b981"/>
  <text x="@{[$panel_x+95]}" y="@{[$main_top+766]}" fill="#ffffff" font-size="16" font-weight="bold" font-family="sans-serif" text-anchor="middle">🏆 權威評分: 6/6分</text>
|;
  ...
  <line x1="230" y1="@{[$h-15]}" x2="250" y2="@{[$h-15]}" stroke="#facc15" stroke-width="2"/>
  <text x="255" y="@{[$h-11]}" fill="#facc15" font-size="11" font-weight="bold" font-family="sans-serif">K線</text>

  <line x1="290" y1="@{[$h-15]}" x2="310" y2="@{[$h-15]}" stroke="#38bdf8" stroke-width="2"/>
  <text x="315" y="@{[$h-11]}" fill="#38bdf8" font-size="11" font-weight="bold" font-family="sans-serif">D線</text>

  <line x1="350" y1="@{[$h-15]}" x2="370" y2="@{[$h-15]}" stroke="#facc15" stroke-width="2"/>
  <text x="375" y="@{[$h-11]}" fill="#facc15" font-size="11" font-weight="bold" font-family="sans-serif">DIF</text>

  <line x1="420" y1="@{[$h-15]}" x2="440" y2="@{[$h-15]}" stroke="#38bdf8" stroke-width="2"/>
  <text x="445" y="@{[$h-11]}" fill="#38bdf8" font-size="11" font-weight="bold" font-family="sans-serif">MACD</text>

  <line x1="500" y1="@{[$h-15]}" x2="520" y2="@{[$h-15]}" stroke="#c084fc" stroke-width="2"/>
  <text x="525" y="@{[$h-11]}" fill="#c084fc" font-size="11" font-weight="bold" font-family="sans-serif">RSI(14)</text>
</svg>|;

    # Save with master versioned filenames
    my $svg_art = "$art_dir/kline_${code}_master.svg";
    my $svg_notes = "$notes_charts_dir/kline_${code}_master.svg";
    ...
    print "Generated MASTER (K-line + Vol + KD + MACD/CD + RSI) SVG for $code $name -> $svg_notes\n";
}

for my $c (@stock_codes) {
    generate_master_indicators_svg($c);
}
