...
def analyze_history(rows:list[dict[str,Any]],end_date:str)->dict[str,Any]:
 ...
 ignition=bool(c[-1]>ph and vr is not None and vr>=Decimal("1.8") and c[-1]>o[-1] and not lu and not fb)
 surge_2x=bool(vr is not None and vr>=Decimal("2.0"))
 risks=[]
 ...
 return {"trade_date":end_date,"history_count":len(rows),"close":T(c[-1]),"ma5":T(ma[5][-1]),"ma10":T(ma[10][-1]),"ma20":T(ma[20][-1]),**up,"bullish_alignment":c[-1]>ma[5][-1]>ma[10][-1]>ma[20][-1],"close_above_ma5":c[-1]>ma[5][-1],"close_above_ma10":c[-1]>ma[10][-1],"close_above_ma20":c[-1]>ma[20][-1],"red_candle":c[-1]>o[-1],"prior_20d_high":T(ph),"distance_from_prior_20d_high_pct":T((c[-1]-ph)/ph*100) if ph>0 else None,"close_breakout_prior_20d_high":c[-1]>ph,"volume_ratio_20d":T(vr),"first_day_ignition":ignition,"volume_surge_2x":surge_2x,"long_upper_shadow":lu,"high_open_low_close":hl,"false_breakout":fb,"risk_flags":risks}
def build(path:Path):
